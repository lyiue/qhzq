...
//						}
//					}
                    $level1_commissions = pdo_fetchall('select *  from ' . tablename('ewei_shop_order_sub') . ' where agent1 = :agentid and `status` = 0 ', array(':agentid' => $member['id']));
                    foreach ($level1_commissions as $c ) {
						if (!empty($c)) {
                            $commission_total += $c['price1'];
						}
					}
				}
...
//					}
				...
				$level1_agentids = pdo_fetchall('select id from ' . tablename('ewei_shop_member') . ' where agentid=:agentid and agentlevel = 5 and isagent=1 and status=1 and uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']), 'id');
...
//							}
//						}
                        $level2_commissions = pdo_fetchall('select *  from ' . tablename('ewei_shop_order_sub') . ' where agent2 = :agentid and `status` = 0  ', array( ':agentid' => $member['id']));
                        foreach ($level2_commissions as $c ) {
                            if (!empty($c)) {
                                $commission_total += $c['price2'];
                            }
                        }
					}
...
//						}
					...
					$level2_agentids = pdo_fetchall('select id from ' . tablename('ewei_shop_member') . ' where agentid = :agentid and agentlevel = 3 and isagent=1 and status=1 and uniacid=:uniacid', array(':agentid' => $member['id'],':uniacid' => $_W['uniacid']), 'id');
...
//							}
//						}
                        $level3_commissions = pdo_fetchall('select *  from ' . tablename('ewei_shop_order_sub') . ' where agent3 = :agentid and `status` = 0 ', array(':agentid' => $member['id']));
                        foreach ($level3_commissions as $c ) {
                            if (!empty($c)) {
                                $commission_total += $c['price3'];
                            }
                        }
					}
...
//						}
					...
					$level3_agentids = pdo_fetchall('select id from ' . tablename('ewei_shop_member') . ' where uniacid=:uniacid and agentid = :agentid and agentlevel = 4 and isagent=1 and status=1', array(':agentid' => $member['id'],':uniacid' => $_W['uniacid']), 'id');
...
//				}
		...
		public function checkAgent($openid = '')
		{
			...
			if ($to_check_agent) {
				pdo_update('ewei_shop_member', array('isagent' => 1, 'status' => $become_check), array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
